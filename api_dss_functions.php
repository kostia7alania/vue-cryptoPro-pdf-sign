<?php  
 ...
 function getStamp ($path, $RectLowerLeftX=33, $RectLowerLeftY = 22, $fixX = 215, $fixY = 110) {
    ...
    try {
       //  $stamp_label = $name . ' ' . $output_array[2].' '.$output_array[3];
     }
    catch (Exception $e) { 
    ...
    $fontSertDesc  = '"Font": { "FontSize": 8, "FontFamily": "times", "FontStyle": 0, "FontColor": {"Red": 0,"Green": 0,"Blue": 0}}';
        ...
        "Content": [ 
            ...
            '{"Text": "Выдан: '.$stamp_label.'",       '.$fontSertDesc.'},    '.$spaceDesc
    ...
    if ( strlen($_POST['cert_base64']) > 30 && strlen($_POST['cert_base64']) < 3000 ) {
        try {    
           ...
           */
            ...
            $parmKeyVal   = [soapVarGen('PDFFormat','CAdEST')];
            $parmKeyVal[] = soapVarGen('CADESType','T');
            $parmKeyVal[] = soapVarGen('TSPAddress','http://testca2.cryptopro.ru/tsp/tsp.srf');
