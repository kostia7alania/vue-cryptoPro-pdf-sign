...
export default {
  ...
  methods: {
    ...
    createSign(stamp_prev) {
      ...
      debugger;
      cryptopro
        .signData(HashValue, Thumbprint) //window.CryptoPro.call('signData', Thumbprint, HashValue)
        .then(
          createdSign => {
          ...
          err => {
            console.warn("CryptoPro.call(signData) => ", err);
            that.echo_end_die({ stat: 0, msg: err });
          }
        );
    },
