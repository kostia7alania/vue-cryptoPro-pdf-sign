<template>
   <div class="appMain">  
            <div class="text-center" v-show="loading_text"> <div> <img src="../img/animaatjes-rubiks-cube.gif"> </div> <div>{{loading_text}}</div>  </div>
     
        <div v-show="!loading_text"> 
            <div class="text-center">
              <app-get-cert-list v-show="!doc_prev && !base64Binary" @sel_cert="msg = '' " @select-position="selectPosition" @loading = "loading_handler"/>
...
export default {
    ...
    mounted(){
        window.axios_instance = axios.create({ headers: { 'crossDomain': true, "Access-Control-Allow-Origin": "*", 'Accept': 'application/json', 'Content-Type': 'application/json;charset=UTF-8', 'responseType': 'json' }/*,responseType: 'json'*/});
        window.CryptoPro = require('./libs/cryptopro_src/index.js')//OLD BRANCH
        //был отключен даже в олдЕ: import './libs/polyfills/atob-btoa.js'
     },
    methods: {
        ...
        this.loading_text='загрузка .... 0/3';
            ...
            .catch( err => {that.loading_text=''; console.log("podpisat catch err=>"+err); that.stat = 0; that.msg = err  });
        },
        createSign(stamp_prev){     console.log("[createSign] [HashValue] => " + this.HashValue)
                ...
                err => { console.error('CryptoPro.call(signData) => ', err ); that.loading_text =''; this.stat=0;this.msg = err; }) 
        },
